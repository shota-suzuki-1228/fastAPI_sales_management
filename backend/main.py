...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    db_user = crud.get_user_by_name_by_password(db, email=user.email, name = user.name)
    if db_user:
...
def read_user_by_name(name: str, db: Session = Depends(get_db)):
    db_user = crud.get_user_by_name(db, user_name=name)
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    return db_user
...
def create_sales(sales: schemas.SalesCreate, db: Session = Depends(get_db)):
    db_sales = crud.get_sales_by_name(db, year=sales.year)
    if db_sales:
        raise HTTPException(status_code=400, detail="Email already registered")
...
def get_sales_by_year(year: int, db: Session = Depends(get_db)):
    db_year = crud.get_user_by_id(db, year=year)
    if db_year is None:
        raise HTTPException(status_code=404, detail="User not found")
    return db_year